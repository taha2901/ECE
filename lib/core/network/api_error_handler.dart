...
ApiErrorModel _parseServerError(DioException error) {
  try {
    ...
    if (data is String) {
      return ApiErrorModel(message: data, code: error.response?.statusCode);
    }
    ...
    if (data is Map<String, dynamic>) {
      ...
      // ✅ validation map ممكن يبقى errors أو fields مباشرة
      Map<String, dynamic> validation = {};
      if (data["errors"] is Map<String, dynamic>) {
        validation = Map<String, dynamic>.from(data["errors"]);
      } else {
        // fields مباشرة
        validation = Map<String, dynamic>.from(data);
        validation.removeWhere((k, _) => ["message", "detail", "error", "status", "code"].contains(k));
      }
